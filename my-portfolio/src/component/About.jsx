...
import { ThemeContext } from '../context/ThemeContext.jsx'; // ThemeContext import karein

const About = () => {
  const { theme } = useContext(ThemeContext); // theme ko context se nikala

  // Card aur text ka color dark/light mode ke hisab se
  const cardClass = theme === 'dark' ? 'bg-dark text-white' : 'bg-white text-dark';
  const textMutedClass = theme === 'dark' ? 'text-light' : 'text-muted';
  const hrColor = theme === 'dark' ? 'rgba(255, 255, 255, 0.2)' : '#ccc';
  const approachCardClass = theme === 'dark' ? 'bg-dark text-white' : 'bg-white text-dark';
  const approachCardBorder = theme === 'dark' ? 'rgba(0, 123, 255, 0.3)' : 'rgba(0, 123, 255, 0.5)';
  ...
  return (
    ...
    >
      <div className="container" data-aos="fade-up">
        ...
        <Row className="align-items-center justify-content-center mb-5">
          <Col lg={7} data-aos="fade-right">
            <p className={`lead fw-normal ${textMutedClass}`} style={{ textAlign: 'justify', textJustify: 'inter-word' }}>
          ...
          <Col lg={4} className="mt-4 mt-lg-0" data-aos="fade-left">
            <Card className={`border-light shadow-lg ${cardClass}`}>
              <Card.Body>
                ...
                <div className="d-flex align-items-center mb-3">
                  ...
                  <div>
                    <h6 className="mb-0 fw-bold">BCA in DS & AI</h6>
                    <p className={`mb-0 small ${textMutedClass}`}>BBD University, Lucknow</p>
                  </div>
                </div>
                <div className="d-flex align-items-center">
                  ...
                  <div>
                    ...
                    <p className={`mb-0 small ${textMutedClass}`}>2026</p>
                  </div>
                </div>
              </Card.Body>
            </Card>
        ...
        <div className="text-center my-5" data-aos="fade-up">
          ...
          <Row>
            <Col md={4} className="mb-4" data-aos="fade-up" data-aos-delay="100">
              <Card className={`h-100 p-3 approach-card ${approachCardClass}`} style={{ border: '1px solid transparent', transition: 'all 0.3s ease' }} onMouseOver={(e) => { e.currentTarget.style.transform = 'translateY(-10px)'; e.currentTarget.style.borderColor = approachCardBorder; }} onMouseOut={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'transparent'; }}>
                <Card.Body>
                  ...
                  <p className={textMutedClass}>I break down complex problems to find the most effective and scalable solutions.</p>
                </Card.Body>
              </Card>
            </Col>
            <Col md={4} className="mb-4" data-aos="fade-up" data-aos-delay="200">
              <Card className={`h-100 p-3 approach-card ${approachCardClass}`} style={{ border: '1px solid transparent', transition: 'all 0.3s ease' }} onMouseOver={(e) => { e.currentTarget.style.transform = 'translateY(-10px)'; e.currentTarget.style.borderColor = approachCardBorder; }} onMouseOut={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'transparent'; }}>
                <Card.Body>
                  ...
                  <p className={textMutedClass}>I believe in writing clean, readable, and maintainable code that follows best practices.</p>
                </Card.Body>
              </Card>
            </Col>
            <Col md={4} className="mb-4" data-aos="fade-up" data-aos-delay="300">
              <Card className={`h-100 p-3 approach-card ${approachCardClass}`} style={{ border: '1px solid transparent', transition: 'all 0.3s ease' }} onMouseOver={(e) => { e.currentTarget.style.transform = 'translateY(-10px)'; e.currentTarget.style.borderColor = approachCardBorder; }} onMouseOut={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'transparent'; }}>
                <Card.Body>
                  ...
                  <p className={textMutedClass}>Always exploring new technologies to stay updated with the latest industry trends.</p>
                </Card.Body>
              </Card>
            </Col>
          </Row>
        ...
        <hr className="my-5" style={{ borderColor: hrColor }} />
                  ...
                  <div className="d-flex justify-content-center flex-wrap" style={{ gap: '1.5rem' }}>
                    {items.map((skill) => (
                      <div key={skill.name} className="text-center" style={{ width: "90px", transition: 'transform 0.2s ease-in-out', cursor: 'pointer' }} title={skill.name} onMouseOver={(e) => { e.currentTarget.style.transform = 'scale(1.15)'; }} onMouseOut={(e) => { e.currentTarget.style.transform = 'scale(1)'; }}>
                        ...
                        <p className={`mt-2 mb-0 small ${textMutedClass}`}>{skill.name}</p>
                      </div>
                    ))}
                  </div>
                </Card.Body>
      ...
      {/* --- Wavy line wala div yahan se hata diya gaya hai --- */}
